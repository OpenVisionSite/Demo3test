...
void main() {
  ...
  // Используем шумовую функцию и время для создания эффекта holo
  ...
  uv.x += direction * (20.0 + 4.0 * lineOffset) / uSize.x;

  vec3 hologram = texture2D(tDiffuse, uv).rgb;
  vec4 color = texture2D(tDiffuse, vUv);

  // Комбинируем цвета голограммы и исходного изображения с интенсивностью holo
  vec3 combinedColor = color.rgb + hologram * holoIntensity;

  // Добавляем синее свечение
